...
def test_trending_species_structure():
    ...
    # 공개 하한: 담은 사람이 적은 종은 집계라도 개인 관심을 드러내므로 내보내지 않는다
    assert all(s["watch_count"] >= tools.MIN_PUBLIC_WATCH for s in r["species"])
...
def test_get_interest_has_watch_count():
    ...
    # 하한 미만은 0으로 가려 나온다 — 1·2 같은 값이 그대로 보이면 개인 관심이 드러난다
    assert r["user_watch_count"] == 0 or r["user_watch_count"] >= tools.MIN_PUBLIC_WATCH
