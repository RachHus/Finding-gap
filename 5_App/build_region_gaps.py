# -*- coding: utf-8 -*-
"""시군구 × 분류군 발견/공백 요약 자산 생성 — demo/data/region_gaps.js

알림과 활동지역 설정이 쓰는 작은 표다. 지도용 격자 자산(cells_*.js)은 종당 수 MB라
알림 한 줄을 만들려고 내려받을 수 없다. 그래서 지역·분류군 단위 합계만 미리 접어 둔다.

소스는 7_MCP/data/fg_mcp.sqlite — 대화형 서비스·지역 프로필 페이지와 같은 자료라
같은 화면 안에서 숫자가 어긋나지 않는다. 발견 정의도 서비스와 동일하게
'기준연도 − 10년' 이내 관측 보유로 맞춘다.

단독 실행: python 5_App/build_region_gaps.py
"""
import json
import sqlite3
